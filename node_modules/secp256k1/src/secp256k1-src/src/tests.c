...
void ec_pubkey_parse_pointtest(const unsigned char *input, int xvalid, int yvalid) {
    ...
    for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            ysign = (input[63] & 1) + 2;
            /* For the current type (i) do we expect parsing to work? Handled all of compressed/uncompressed/hybrix. */
...
void test_random_pubkeys(void) {
    ...
    if (secp256k1_eckey_pubkey_parse(&elem, in, len)) {
        ...
        /* ... except for the type of hybrix inputs. */
        if ((in[0] != 6) && (in[0] != 7)) {
            CHECK(in[0] == out[0]);
        }
        size = 65;
        ...
        /* Check that the X9.62 hybrix type is checked. */
