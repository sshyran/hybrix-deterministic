...
let VALUE_UINT64_MAX = Buffer.from('ffffffffffffffff', 'hex');
let BLANK_OUTPUT = {
  script: EMPTY_SCRIPT,
...
Transaction.fromBuffer = function (buffer, network = networks.bitcoin, __noStrict) {
  ...
  function readInt64 () {
    let i = bufferutils.readInt64LE(buffer, offset);
    offset += 8;
    return i;
  }

  function readUInt64 () {
    let i = bufferutils.readUInt64LE(buffer, offset);
    offset += 8;
    return i;
  }
  ...
  function readJoinSplit () {
    let vpubOld = readUInt64();
    let vpubNew = readUInt64();
    let anchor = readSlice(32);
    let nullifiers = [];
    ...
    let ephemeralKey = readSlice(32);
    let randomSeed = readSlice(32);
    let macs = [];
    for (j = 0; j < Transaction.ZCASH_NUM_JOINSPLITS_INPUTS; j++) {
      macs.push(readSlice(32));
    }

    let zkproof = readZKProof();
    let ciphertexts = [];
    for (j = 0; j < Transaction.ZCASH_NUM_JOINSPLITS_OUTPUTS; j++) {
  ...
  function readShieldedSpend () {
    let cv = readSlice(32);
    let anchor = readSlice(32);
    let nullifier = readSlice(32);
    let rk = readSlice(32);
    let zkproof = readZKProof();
    let spendAuthSig = readSlice(64);
    return {
      cv: cv,
      anchor: anchor,
      nullifier: nullifier,
  ...
  function readShieldedOutput () {
    let cv = readSlice(32);
    let cmu = readSlice(32);
    let ephemeralKey = readSlice(32);
    let encCiphertext = readSlice(580);
    let outCiphertext = readSlice(80);
    let zkproof = readZKProof();
  ...
  let flag = buffer.readUInt8(offset + 1);
...
Transaction.prototype.weight = function () {
  let base = this.__byteLength(false);
  let total = this.__byteLength(true);
  return base * 3 + total;
};
...
Transaction.prototype.getJoinSplitByteLength = function () {
  ...
  let joinSplitsLen = this.joinsplits.length;
  let byteLength = 0;
...
Transaction.prototype.__byteLength = function (__allowWitness) {
  let hasWitnesses = __allowWitness && this.hasWitnesses();
...
Transaction.prototype.clone = function () {
  let newTx = new Transaction(this.network);
...
Transaction.prototype.getHeader = function () {
  let mask = (this.overwintered ? 1 : 0);
  let header = this.version | (mask << 31);
  return header;
};
...
Transaction.prototype.hashForSignature = function (inIndex, prevOutScript, hashType) {
  ...
  let ourScript = bscript.compile(bscript.decompile(prevOutScript).filter(function (x) {
  ...
  // serialize and hash
  let buffer = Buffer.allocUnsafe(txTmp.__byteLength(false) + 4);
...
Transaction.prototype.getBlake2bHash = function (bufferToHash, personalization) {
  ...
  console.log('Buffer.from(personalization)).update(bufferToHash).digest(out) = ', Buffer.from(personalization)).update(bufferToHash).digest(out);
  return blake2bjs(Buffer.from(personalization), null, out.length).update(bufferToHash).digest(out);
};
...
Transaction.prototype.getPrevoutHash = function (hashType) {
  if (!(hashType & Transaction.SIGHASH_ANYONECANPAY)) {
    let bufferWriter = new BufferWriter(36 * this.ins.length);
...
Transaction.prototype.getSequenceHash = function (hashType) {
  if (!(hashType & Transaction.SIGHASH_ANYONECANPAY) &&
    ...
    let bufferWriter = new BufferWriter(4 * this.ins.length);
...
Transaction.prototype.getOutputsHash = function (hashType, inIndex) {
  ...
  if ((hashType & 0x1f) !== Transaction.SIGHASH_SINGLE && (hashType & 0x1f) !== Transaction.SIGHASH_NONE) {
    ...
    let txOutsSize = this.outs.reduce(function (sum, output) {
...
Transaction.prototype.hashForZcashSignature = function (inIndex, prevOutScript, value, hashType) {
  ...
  if (this.isOverwinterCompatible()) {
    let hashPrevouts = this.getPrevoutHash(hashType);
    let hashSequence = this.getSequenceHash(hashType);
    let hashOutputs = this.getOutputsHash(hashType, inIndex);
...
Transaction.prototype.hashForWitnessV0 = function (inIndex, prevOutScript, value, hashType) {
  ...
  let bufferWriter = new BufferWriter(156 + varSliceSize(prevOutScript));
  let input = this.ins[inIndex];
...
Transaction.prototype.hashForGoldSignature = function (inIndex, prevOutScript, inAmount, hashType, sigVersion) {
  ...
  let fUseForkId = (hashType & Transaction.SIGHASH_BITCOINCASHBIP143) > 0;
  if (fUseForkId) {
