...
function sign (transaction, keys) {
        ...
        var signature = window.nacl.crypto_sign_detached(hash, Buffer.from(keys.privateKey, 'hex'));
...
function verify (transaction) {
        ...
        var res = window.nacl.crypto_sign_verify_detached(signatureBuffer, hash, senderPublicKeyBuffer);
...
function verifySecondSignature (transaction, publicKey) {
        ...
        var publicKeyBuffer = Buffer.from(publicKey, 'hex');
        var res = window.nacl.crypto_sign_verify_detached(signSignatureBuffer, hash, publicKeyBuffer);
...
function getKeys (secret) {
        ...
        // This does not work within Webpack bundle. Need to prepend 'window.'
        var keypair = window.nacl.crypto_sign_keypair_from_seed(hash);
