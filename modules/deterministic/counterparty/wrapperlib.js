...
CWHierarchicalKey.prototype.init = function(passphrase) {
  ...
  //  var words = $.trim(passphrase.toLowerCase()).split(' ');
    var words = passphrase.toLowerCase().split(' ');
   ...
   */
  this.oldHierarchicalKey = bitcore.HDPrivateKey.fromSeed(bitcore.deps.Buffer(seed, 'ascii'), NETWORK);
...
CWHierarchicalKey.wordsToSeed = function(words) {
  var m = new mnemonic.Mnemonic(words);
  return m.toHex();
}
