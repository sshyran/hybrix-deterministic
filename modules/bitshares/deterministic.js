...
function webSocketCall (request, dataCallback, errorCallback) {
  let webSocket = new WebSocket('wss://bitshares.crypto.fans/ws');
  webSocket.onopen = function () {
    let loginRequest = {'method': 'call', 'params': [1, 'login', ['', '']], 'id': 1};
  ...
  function () {
    let functions = {
      ...
      validate: (address, callback) => {
        ...
        let request = {'id': 2, 'method': 'call', 'params': [0, 'get_account_by_name', [address]]};
...
*/
      ...
      generate: (data, callback) => {
        ...
        let activeMemoKey = publickKey;
        let ownerKey = publickKey;
        let name = address;
        let request = {account: {name: name, owner_key: ownerKey, active_key: activeMemoKey, memo_key: activeMemoKey, refcode: null, referrer: null}};
        httpCall(request);
    ...
    "base": [
      ...
      transaction: (data, callback) => {
        ...
        if (typeof data.unspent !== 'undefined' && typeof data.unspent[0] !== 'undefined' && typeof data.unspent[0][0] !== 'undefined') {
          ...
          let target = data.unspent.target;
          let assetID = data.contract;
          ...
          */
          ...
          let rawtxstring = JSON.stringify(tr.serialize());
